/**
*
* @Title: infuse_play.js
*
* @Description: iOS 浏览器中调用 Infuse 播放器播放
*
* @author: Arcade
*
* @version V1.0
      ...
      if (error) {
        ...
        for (let key in video_data.MediaSources) {
          ...
          if (media_source.Id == media_source_id) {
            ...
            let infuse_url_scheme = generateInfuseURLScheme(download_info, api_key);    // 生成 Infuse URL Scheme 地址
...
function generateInfuseURLScheme(data, api_key) {
  let infuse_x_callback = "infuse://x-callback-url/play?url=" + encodeURIComponent(data.video.url + "&api_key=" + api_key);

  // Infuse 的 x-callback-url 不支持字幕 URL
  /*for (let key in data.subtitles) {
    infuse_x_callback += "&sub=" + encodeURIComponent(data.subtitles[key].url + "?api_key=" + api_key);
    break;
  }*/

  return infuse_x_callback;
}
