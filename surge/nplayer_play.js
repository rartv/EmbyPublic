/**
*
* @Title: nplayer_play.js
*
* @Description: iOS 浏览器中调用 nPlayer 播放器播放
*
* @author: Arcade
*
* @version V1.0
      ...
      if (error) {
        ...
        for (let key in video_data.MediaSources) {
          ...
          if (media_source.Id == media_source_id) {
            ...
            let nplayer_url_scheme = generateNplayerURLScheme(download_info, api_key);    // 生成 nPlayer URL Scheme 地址
...
function generateNplayerURLScheme(data, api_key) {
  let nplayer_url_scheme = "nplayer-" + data.video.url + "&api_key=" + api_key;

  // nPlayer 的 URL Scheme 不支持字幕 URL

  return nplayer_url_scheme;
}
